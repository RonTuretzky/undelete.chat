...
Use `docker compose -f deploy/compose.yaml up -d --build` to update. Do not use `down -v`: it deletes the archive volume. Production creates an online SQLite backup at startup and every 24 hours, retaining the latest seven snapshots in `/data/backups`. Snapshots contain encrypted collector queues and the archive; the encryption key must be backed up separately. `node server/backup.mjs` creates a manual snapshot. Backups on the same volume do not protect against total server loss; off-site replication remains required before public launch. The initial deployment does not automatically enable paid DigitalOcean backups. No billing/subscription integration, email delivery, or multi-region service is configured.
