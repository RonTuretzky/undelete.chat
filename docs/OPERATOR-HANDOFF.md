...
| Invitation gate (off) | Optional `invite_code` field in `app-secrets.json`; blank since September 11, so registration is open and every workspace starts a 14-day trial | Allowing new registrations while the service is in private beta | Keep private. Removing the invitation requirement requires an explicit product decision and deployment change. |
