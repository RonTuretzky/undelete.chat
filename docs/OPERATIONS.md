...
2. Download a complete application snapshot and check its manifest. Restore into a new, private directory. Use the matching archive key to run SQLite integrity checks and decrypt representative session records without starting collectors. Message records are sealed to each account's own key since September 14, 2026, so the server key cannot open them; verify them by envelope format, counts, and integrity rather than by decryption, or use an account's password with the browser vault module under Node.
...
- Data: message records sealed to a per-account P-256 key held only by the owner (ephemeral ECDH, HKDF, AES-256-GCM) with the wrapped private key stored server-side, older data AES-256-GCM under the server key until re-sealed, per-collector keys derived from the server key and passed only over IPC, `secure_delete` with WAL truncation after purges and deletions, a restrictive umask, and a 0700 data volume. Backups keep deleted data for up to seven days, which the privacy policy states.
...
Accepted risks: the server sees each message transiently at capture (stored content is sealed to the account's own key since September 14 and cannot be read by operators; see Premium for moving capture into an enclave); full-server backup images include the key; the registration endpoint reveals whether a username is taken; the production image is built on the server from the npm registry rather than from a reviewed artifact.
...
The native store apps (`mobile/`) receive notifications through Apple's push service and Firebase Cloud Messaging. Both are configured from private `~/.config/afterword/native-push-config.json`: an `apns` object (`key_id`, `team_id`, `bundle_id`, `key_file` pointing at the `.p8` key, optional `sandbox`) and `fcm_service_account_file` naming the Firebase service account JSON. `deploy.py` turns these into `APNS_*` and `FCM_SERVICE_ACCOUNT` environment values. Device tokens are stored per account (newest ten), never echoed by the API, dropped when Apple or Google report them unregistered, and removed after twenty failures. Until the file exists the server logs that native push is not configured and the apps' Settings say so. See `mobile/README.md` for the account setup and store submission steps.
...
- Kernel updates: unattended-upgrades installs security patches daily and, since September 12, reboots automatically at 09:30 UTC when a kernel or libc update requires it (`/etc/apt/apt.conf.d/52afterword-reboot`, also in `provision.py`). The reboot takes about a minute; the supervisor restores every hosted session afterwards and the outage alert may fire once. `python3 deploy/operations.py maintain` runs the rest of the monthly pass: package upgrades, Docker cleanup, a reboot-required check (`--reboot` reboots immediately), and a report of outdated client libraries with the next steps.
...
- Dormant accounts: a workspace with no sign-in for `DORMANT_DAYS` (default 365) and no active, trialing, or past-due subscription is erased by a sweep that runs every six hours, including its sessions, hosted collectors, and archive. Sign-in activity is recorded to the hour. Set `DORMANT_DAYS=0` to disable. The terms and privacy policy state the rule.
