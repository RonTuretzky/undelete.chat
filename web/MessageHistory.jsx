  ...
  return <>
    ...
    <aside ref={panel} className="detail-panel" role="dialog" aria-modal="true" aria-label="Message history">
      ...
      {message && <>
        ...
        {message.status === 'deleted' && <div className="deleted-notice"><Trash2 size={16}/><div><strong>Deleted from {names[message.platform]}</strong><p>{message.originalMissing ? 'The original content was not captured. This happens when the message arrived before this account was linked, while the connection was offline, or as view-once media, which is never kept.' : 'Your captured copy is still in your archive.'}{message.versions?.some(v => v.disappearing) ? ' This chat uses disappearing messages; the copy here stays until you remove it.' : ''}</p></div></div>}
