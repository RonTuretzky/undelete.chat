...
function HostedSetup({ platform, available, initialConnection, Modal, Platform, api, onClose, onFinish, onChanged, onBack, onBrowser }) {
  ...
  return <Modal title={connected ? `${info.name} is connected` : `Connect ${info.name}`} wide onClose={onClose}>
    ...
    {setup?.capacityReason && <p className="wizard-smallprint">Existing queued copies are kept. Resolve the storage issue, then use Try again to resume your saved session. Activity received while capture is stopped may be missed. <a href="/docs/privacy#allowance" target="_blank" rel="noreferrer">Manage archive storage</a></p>}
